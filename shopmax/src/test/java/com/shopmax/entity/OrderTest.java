...
@SpringBootTest
@Transactional //트랜잭션 처리: 중간에 에러가 발생시 rollback 을 시켜준다.
@Rollback(false)
...
//        item.setUpdateTime(LocalDateTime.now());
    ...
    public void cascadeTest() {
        ...
        em.clear(); //DB 에서 select 하는 쿼리문을 실행시키기 위해 엔티티를 영속성 컨텍스트에 분리한다.
...
//        System.out.println(savedOrder); //메모리 부족
    ...
    public Order createOrder () {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            OrderItem orderItem = new OrderItem();
            orderItem.setItem(item); //orderItem 객체가 참조하는 item 을 넣어준다.
            orderItem.setCount(10);
            orderItem.setOrderPrice(1000);
            orderItem.setOrder(order); //orderItem 객체가 참조하는 order 를 넣어준다.
            order.getOrderItems().add(orderItem); //order 객체가 참조하는 orderItem 을 넣어준다.
        }

        Member member = new Member();
        memberRepository.save(member);

        order.setMember(member); //order 가 참조하는 member 를 넣어준다.
        orderRepository.save(order);
